...
sicm_arena sicm_arena_create(size_t sz, sicm_device *dev) {
	...
	sa->mutex = (pthread_mutex_t *) mmap(NULL, sizeof(pthread_mutex_t), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE, -1, 0);
...
static void sicm_arena_range_move(void *aux, void *start, void *end) {
	...
	err = mbind((void *) start, (char*) end - (char*) start, MPOL_BIND, nodemask->maskp, nodemask->size, MPOL_MF_MOVE);
	if (err < 0 && sa->err == 0)
		sa->err = err;
...
void *sicm_alloc(size_t sz) {
	...
	if (sa != NULL)
	{
		// check if the allocation is where it is supposed to be
		int pnum = 2 + sz / 4096;
		int *status = malloc(pnum * sizeof(int));
		void **pages = malloc(pnum * sizeof(void*));

		uintptr_t p = (uintptr_t) ret;
		uintptr_t ep = p + sz;
		int pcnt;

		p -= p%4096;
		for(pcnt = 0; p < ep; pcnt++) {
			pages[pcnt] = (void *) p;
			p += 4096;
		}

		if (move_pages(0, pcnt, pages, NULL, status, 0) < 0) {
			fprintf(stderr, "move_pages failed: %d\n", errno);
		}

		int wrong = 0;
		for(int i = 0; i < pcnt; i++) {
			if (status[i] != sa->numaid)
				wrong++;
		}
		fprintf(stderr, "sicm_alloc: numa %d total %d wrong %d\n", sa->numaid, pcnt, wrong);
...
success:
	if (mbind(ret, size, MPOL_BIND, nodemask->maskp, nodemask->size, MPOL_MF_MOVE) < 0) {
		munmap(ret, size);
		ret = NULL;
		goto restore_mempolicy;
	}
